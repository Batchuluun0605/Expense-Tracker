...
function CreateIncomes({ refreshData }) {
  ...
  const { user } = useUser();
  const onCreateIncomes = () => {
    const newIncome = { emojiIcon, name, amount };
    const existingIncomes = cookies.incomes ? cookies.incomes : [];
    const updatedIncomes = [...existingIncomes, newIncome];
    setCookie("incomes", JSON.stringify(updatedIncomes), {
      maxAge: 600 * 24 * 60 * 60,
      path: "/",
    });
  };
